...
class DatabaseManager:
    ...
    def get_user_workflows(self, user_id: str):
        """사용자의 모든 워크플로우 목록 조회 (workflow_id와 제목만 반환)"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def get_workflow_by_id(self, n8n_workflow_id: str, user_id: str):
        ...
        cursor.execute('''
            SELECT n8n_workflow_id, status, created_at, updated_at
            FROM workflows 
            WHERE n8n_workflow_id = ? AND user_id = ?
        ''', (n8n_workflow_id, user_id))
...
class ClaudeMCPBackend:
    ...
    async def chat_stream(self, content, user_id: str, session_id: str = None):
        ...
        # 디버깅: 받은 content 출력
        print(f"[DEBUG] chat_stream received content: {content}")
        print(f"[DEBUG] content type: {type(content)}")
        if isinstance(content, list):
            print(f"[DEBUG] content length: {len(content)}")
            for i, block in enumerate(content):
                print(f"[DEBUG] content[{i}]: {block}")
        else:
            print(f"[DEBUG] content is not a list!")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                print(f"[INFO] n8n 워크플로우 이름 업데이트 성공: {workflow_id} -> {name}")
                return True
            else:
                print(f"[ERROR] n8n 워크플로우 이름 업데이트 실패: {response.status_code} - {response.text}")
                return False
